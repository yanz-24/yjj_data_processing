...
def get_df_cat(data_type):
    ...
    '''
    if data_type == "体外诊断试剂":
        df_in_vitro = pd.read_excel('_data/外诊断试剂分类子目录.xls', index_col=0, header=0)
        name_list = [] # 被测物质的名称
        usage_list = [] # 用途
        ...
        df_cat = pd.read_excel("_data/医疗器械分类目录21和22（2023-手动修改-最终版）.xlsx", sheet_name=data_type) # 分类目录
